...
(function(factory) {
  ...
  DmUploader.prototype.addFiles = function(files)
  {
    ...
    {
      var file = this.validateFile((typeof doc !== "undefined")?files[i].file:files[i]);
      let fileName = null;
      ...
      if(typeof doc !== "undefined"){
        file.widget.settings.extraData = files[i].meta;
        fileName = files[i].meta.name;
      }

      // If the callback returns false file will not be processed. This may allow some customization
      var can_continue = this.settings.onNewFile.call(this.element, file.id, file.data,fileName);
  ...
  $.fn.dmUploader = function (n) {
    let i = arguments;
    if ("string" != typeof n)
      return this.each(function () {
        $.data(this, pluginName) || $.data(this, pluginName, new DmUploader(this, n));
      });
    this.each(function () {
      const s = $.data(this, pluginName);
      s instanceof DmUploader
        ? "function" == typeof s.methods[n]
          ? s.methods[n].apply(s, Array.prototype.slice.call(i, 1))
          : $.error("Method " + n + " does not exist in jQuery.dmUploader")
